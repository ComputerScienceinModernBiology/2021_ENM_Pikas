...
setwd ("C:/Users/jezkovt/Dropbox/ENM Pikas") #make sure you navigate to the directory where your variables are
...
VARIABLES_LGM <- list.files (path="./cclgmbi_2-5m", pattern='tif', full.names=TRUE) #fyi, there is an error in this line of code just to keep things interesting
...
VARIABLES_FUTURE <- list.files (path="./ac45bi70", pattern='tif', full.names=TRUE) #fyi, there is an error in this line of code just to keep things interesting
WC_FUTURE <- stack (VARIABLES_FUTURE)

# all variables have to be the exactly the same extent and resolution. These are conveniently the same resolution 
WC_FUTURE <- crop (WC_FUTURE, r)

# now we project the niche on the LGM climate
rfuture <- predict(WC_FUTURE, xm, progress='text')
...
plot (STATES, add=T)
plot (wrld_simpl, add=T)
points (THINNED, cex=1, col='green')
plot(rfuture)
plot (STATES, add=T)
plot (wrld_simpl, add=T)
points (THINNED, cex=1, col='blue')
...
rgap <- predict(wcgap, xm, progress='text')

#this is how we can plot the model
plot(rgap)
plot (STATES, add=T)
plot (wrld_simpl, add=T)
points (RECORDS, cex=1, col='black')
# compare this model to the one built using climate alone. What do you conclude?


#### LET's EXPLORE THE MAXENT RESULTS ####

# we will learn about AUC, variable importance, and tresholds

xm # this will bring Maxent output

#### AUC video https://www.youtube.com/watch?v=OAl6eAyP-yo 

# what is going to make our AUC values worse, let's try few records
RECORDS <- read.csv ("O_princeps_thin1_10records.csv")
RECORDS <- RECORDS [,-1]
xm <- maxent (WC_cropped, RECORDS, nbg=10000, removeDuplicates=TRUE, args=c('betamultiplier=2.5','linear=true', 'quadratic=true', 'hinge=true', 'product=true'))
xm

#### now, we will explore different tresholds
xm <- maxent (WC_cropped, LONGLAT_THINNED, nbg=10000, removeDuplicates=TRUE, args=c('betamultiplier=2.5','linear=true', 'quadratic=true', 'hinge=true', 'product=true'))
r <- predict(WC_cropped, xm, progress='text')

r_tres <- r > xm@results['Equal.training.sensitivity.and.specificity.Cloglog.threshold',]
r_tres2 <- r > xm@results['Minimum.training.presence.Cloglog.threshold',]
r_tres3 <- r > xm@results['Balance.training.omission..predicted.area.and.threshold.value.Cloglog.threshold',]

data (wrld_simpl)
dev.new(width=40, height=12)
par(mfrow=c(1,3))
plot(r_tres)
plot (STATES, add=T)
plot (wrld_simpl, add=T)
...
# What can you conclude about different tresholds?

#### variable importance

# we will explore variable contribution and jackknife

xm <- maxent (WC_cropped, LONGLAT_THINNED, nbg=10000, removeDuplicates=TRUE, args=c('jackknife=true', 'betamultiplier=2.5','linear=true', 'quadratic=true', 'hinge=true', 'product=true'))
xm

#### let's explore some of the parameters, e.g., beta multiplier ####

xm <- maxent (WC_cropped, LONGLAT_THINNED, nbg=10000, removeDuplicates=TRUE, args=c('betamultiplier=0.5','linear=true', 'quadratic=true', 'hinge=true', 'product=true'))
r1 <- predict(WC_cropped, xm, progress='text')

xm <- maxent (WC_cropped, LONGLAT_THINNED, nbg=10000, removeDuplicates=TRUE, args=c('betamultiplier=8','linear=true', 'quadratic=true', 'hinge=true', 'product=true'))
r2 <- predict(WC_cropped, xm, progress='text')
...
#again, what do you concldue about the beta multiplier
